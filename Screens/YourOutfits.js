...
import { StyleSheet, Text, View, TouchableOpacity, Modal, TextInput, FlatList, Image, ScrollView } from 'react-native';
...
export default class YourOutfits extends React.Component {
    constructor() {
        super()
        this.state = {
            userId: 'saanvisinha261048@gmail.com',
            imageURI: '',
            style: '',
            imageId: '',
    ...
    uploadImage = async (uri) => {
        ...
        var ref = firebase.storage().ref().child("user_profiles/" + imageID)
      ...
      getAllOutfits = () => {
        db.collection("Outfits").where("user_id","==",this.state.userId).onSnapshot((snapshot) => {
      ...
      if(this.state.modalVisibility == true) {
        ...
        <View style={styles.container}>
          <ScrollView>
            ...
            <View>
              {this.state.outfits.length == 0 ? (<Text>Upload your outfits!</Text>) : (
              <FlatList 
